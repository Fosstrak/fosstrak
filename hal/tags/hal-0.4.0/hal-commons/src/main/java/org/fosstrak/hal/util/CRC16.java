...
public class CRC16 {
	...
	static public byte[] crc16(byte[] data) {
		...
		for(int i=0; i<data.length; i++) {
			ch = Integer.parseInt(ByteBlock.byteToHexString(data[i]), 16); 
			crc = (crc >> 8 ) ^ CRCtbl[(crc & 0xFF ) ^ ch]; 
		}
		int low=crc & 0xFF;
		int high=crc >> 8;
		byte[] resp = new byte[2];
		resp[0] = ByteBlock.numberToByteArray(low)[3];
		resp[1] = ByteBlock.numberToByteArray(high)[3];
		return resp; 
	}
